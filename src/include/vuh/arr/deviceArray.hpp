...
namespace vuh {
	namespace arr {
		...
		class DeviceOnlyArray: virtual public BasicArray<T, Alloc> {
		public:
			using value_type = T;
		...
		/// doc me
		template<class T, class Alloc>
		auto device_begin(DeviceArray<T, Alloc>& arr)-> ArrayIter<DeviceArray<T, Alloc>> {
			return arr.device_begin();
		}

		/// doc me
		template<class T, class Alloc>
		auto device_end(DeviceArray<T, Alloc>& arr)-> ArrayIter<DeviceArray<T, Alloc>> {
			return arr.device_end();
		}
	} // namespace arr
} // namespace vuh
