...
TEST_CASE("saxpy_once", "[correctness]"){
	...
	auto kernel = vuh::Kernel<params_t, arrays_t>("../shaders/saxpy.spv"); // define the kernel by linking interface and spir-v implementation
	kernel.on(device).grid(128/64).spec(64)({128, a}, d_y, d_x); // run once, wait for completion
